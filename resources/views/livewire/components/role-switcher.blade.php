<div class="relative" x-data="{}" @keydown.escape.window="$wire.open = false" @click.outside="$wire.open = false">
    @if ($currentUser)
        <button type="button" wire:click="toggle"
            ...
            <span class="grid size-7 place-items-center rounded-full bg-emerald-600 text-xs font-semibold text-white">
    ...
    @else
        <button type="button" wire:click="toggle"
                class="rounded-full bg-emerald-600 px-4 py-1.5 text-sm font-semibold text-white shadow-xs transition hover:bg-emerald-700">
            Sign in to demo
        </button>
    @endif
    ...
    @if ($open)
        <div x-transition.opacity
            ...
            <div class="px-3 py-2 text-[11px] font-semibold uppercase tracking-wider text-zinc-500">
                ...
                @foreach ($users as $user)
                    <li>
                        <button type="button" wire:click="signInAs({{ $user->id }})"
                            ...
                            <span class="grid size-8 place-items-center rounded-full bg-zinc-100 text-xs font-semibold text-zinc-700">
                                {{ $user->initials() }}
                            </span>
